...
/**
 * Create order with order_date parameter
 */
function create_order_with_date_ctr($customer_id, $invoice_no, $order_date, $order_status = 'Pending') {
    $order = new Order();
    // Your Order class needs to support this - use existing create_order method
    return $order->create_order($customer_id, $invoice_no, $order_status);
}

/**
 * Record payment with extended parameters for Paystack
 */
function record_payment_extended_ctr($amount, $customer_id, $order_id, $currency, $order_date, $payment_method, $reference, $authorization_code = null, $channel = null) {
    $order = new Order();
